...
def MEG_import_process(file, out, destination_mat):      
    ...
    raw_data_sss = mne.preprocessing.maxwell_filter(
       ...
       head_pos= chpi_pos, 
       verbose=True,
       calibration = root_path / 'Tools' / 'sss_cal.dat',
       cross_talk = root_path / 'Tools' / 'ct_sparse.fif'
       )
...
def import_ER(date):
    ...
    raw_data_sss = mne.preprocessing.maxwell_filter(
       ...
       destination= None, 
       verbose=True,
       calibration = base_path / 'Tools' / 'sss_cal.dat',
       cross_talk = base_path / 'Tools' / 'ct_sparse.fif'
       )
